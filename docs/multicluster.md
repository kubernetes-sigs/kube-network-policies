...
### 4. Example: Cross-cluster Network Policy

Let's deploy a `web` service in `cluster-a` and then create a network policy
that allows traffic only from a specific pod in `cluster-b`.

Deploy the `web` service in `cluster-a`:

```bash
# Replace <context-for-cluster-a> with the kubectl context for your first cluster
kubectl --context <context-for-cluster-a> run web --image=httpd:2 --labels="app=web" --expose --port=80
kubectl --context <context-for-cluster-a> wait --for=condition=ready pod -l app=web --timeout=2m
```

Now, apply a network policy that allows ingress traffic to the `web` pod only
from pods with the label `app=client` in `cluster-b`:

```bash
# Replace <context-for-cluster-a> and <cluster-b-name> with your values
SERVICE_IP_A=$(kubectl --context <context-for-cluster-a> get svc web -o jsonpath='{.spec.clusterIP}')
...
spec:
  ...
  - from:
    - podSelector:
      ...
      namespaceSelector:
        matchLabels:
          networking.k8s.io/multicluster-name: <cluster-b-name>
EOF
```
...
```bash
# Replace <context-for-cluster-b> with the kubectl context for your second cluster
kubectl --context <context-for-cluster-b> run client-allowed --image=busybox --labels="app=client" --rm -it --restart=Never --command -- wget -O- --timeout=2 http://${SERVICE_IP_A}
```
...
```bash
# Replace <context-for-cluster-b> with the kubectl context for your second cluster
kubectl --context <context-for-cluster-b> run client-denied --image=busybox --rm -it --restart=Never --command -- wget -O- --timeout=2 http://${SERVICE_IP_A}
```
