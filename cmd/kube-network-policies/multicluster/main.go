...
import (
	...
	pluginsmulticluster "sigs.k8s.io/kube-network-policies/plugins/multicluster"
...
func run() int {
	...
	if err != nil {
		klog.Fatalf("Failed to get local kube-system namespace: %v", err)
	}
	localClusterID := string(nsKubeSystem.UID)
	klog.Infof("Determined local cluster ID: %s", localClusterID)
	...
	*/
	...
	evaluators = append(evaluators, pluginsmulticluster.NewMultiClusterNetworkPolicy(networkPolicyInfomer, localClusterID))
