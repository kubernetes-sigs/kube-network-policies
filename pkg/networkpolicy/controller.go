...
func (c *Controller) syncNFTablesRules(ctx context.Context) error {
	...
	// Allow all ICMPv6 types except Echo Request (128) and Echo Reply (129)
	// which should be processed by network policies
	...
	nft.AddRule(&nftables.Rule{
		...
		Exprs: []expr.Any{
			...
			&expr.Cmp{Op: expr.CmpOpNeq, Register: 0x1, Data: []uint8{128}},
			&expr.Payload{DestRegister: 0x1, Base: expr.PayloadBaseTransportHeader, Offset: 0, Len: 1},
			&expr.Cmp{Op: expr.CmpOpNeq, Register: 0x1, Data: []uint8{129}},
			&expr.Verdict{Kind: expr.VerdictAccept},
		},
	})
